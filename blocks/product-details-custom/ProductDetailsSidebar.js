    ...
    return values.map((value) => {
      ...
      return html`<li key=${value.id} class="swatch">
        <button
          ...
          onClick=${() => updateSelection({ [option.id]: value })}
        >
          ${value.title}
        </button>
      </li>`;
  ...
  return options.map((option) => {
    ...
    return html`<div
      class=${`sidebar-section ${
        option.id === "color" ? "color-selector" : "sizes-selector"
      }`}
    >
      <h4>
        ${selectedTitle ? `${option.title}: ${selectedTitle}` : option.title}
      </h4>
      <ul class="swatches swatches-regular">
        ${renderSwatches(option)}
      </ul>
    </div>`;
  });
}
