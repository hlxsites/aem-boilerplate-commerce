...
- **Block-Specific Permission Check**: For this block, checks if user has admin or `Magento_PurchaseOrder::view_purchase_orders_for_company` permission (via `PO_PERMISSIONS.VIEW_COMPANY` constant)
...
5. **Block-Specific Permission Check**: Checks for admin or company-wide purchase order permission (`VIEW_COMPANY`) for this specific block
...
7. **Orders Display**: If authorized, renders company purchase orders with pagination using default dropin configuration
