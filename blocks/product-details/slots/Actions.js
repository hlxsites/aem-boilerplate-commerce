...
import { addToCart } from '../product-details.js';
...
export default function Actions(ctx) {
  ...
  ctx.appendButton((state) => {
    return {
      type: 'submit',
      innerHTML: 'Add to Cart',
      onClick: async () => {
        const sku = ctx.state.product.sku;
        const quantity = 1;
        const optionsUIDs = [];

        try {
          await addToCart({ sku, quantity, optionsUIDs });
  ...
  events.on('eds/lcp', () => {
    const cartButton = document.querySelector(
      '.pdp-product__buttons > button:nth-child(1)'
    );
    ...
    const x = setInterval(function () {
      ...
      if (hours > 0) {
        document.querySelector(
          `[data-test-id='ShippingMessage__OrderText']`
        ).innerHTML = 'Order within ' + hours + ' hours ' + minutes + ' mins';
      } else {
        document.querySelector(
          `[data-test-id='ShippingMessage__OrderText']`
        ).innerHTML = 'Order within ' + minutes + ' mins';
      }
      ...
      document.querySelector(
        `[data-test-id='ShippingMessage__DeliveryEstimate']`
      ).innerHTML = day + ' ' + tomorrow.getDate() + ' ' + month;
    });
  });
}
