...
/**
 * Fetches nav categories from an EDS spreadsheet JSON and builds a nav-sections
 * DOM subtree compatible with the 2-level submenu the boilerplate CSS and
 * setupSubmenu() expect:
 *
 *   <ul>                         ← top-level bar items
 *     <li>                       ← e.g. Women
 *       <a href="/women">Women</a>
 *       <ul>                     ← flat dropdown list
 *         <li><a>Women's Clothing</a></li>
 *         <li><a>Women's Pants</a></li>   ← grandchildren flattened in
 *         ...
 *       </ul>
 *     </li>
 *   </ul>
 *
 * Depth-1 rows (single path segment) become top-level bar items.
 * All deeper rows are flattened into their nearest depth-1 ancestor's dropdown,
 * preserving full paths as link hrefs for correct routing.
 *
 * @param {string} src  URL of the nav-categories spreadsheet JSON
...
export default async function decorate(block) {
  ...
  // Replace static nav sections with dynamically synced category data.
  // Falls back to whatever is in the /nav document if the fetch fails or returns nothing.
