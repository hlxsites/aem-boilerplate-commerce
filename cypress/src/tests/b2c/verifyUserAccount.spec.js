...
describe('Verify user account functionality', () => {
  it('Verify auth user can create addresses', { tags: '@snapPercy' }, () => {
    ...
    cy.contains("Your account information has been updated.").should(
      "be.visible",
    );
    ...
    cy.contains("Are you sure you would like to remove this address?").should(
      "be.visible",
    );
    cy.get(".account-address-modal__buttons")
      .find("span")
      .contains("Remove")
    ...
    cy.contains("Change password")
      .should("not.be.disabled")
      .click({ force: true });
    cy.get('[data-testid="toggle-password-icon"]')
      .first()
      .click({ force: true });
    ...
    cy.contains("Manage your returns")
      .should("not.be.disabled")
      .click({ force: true });
