...
describe('Search Feature', () => {
  ...
  it(
    "Verify Filter on search results page",
    { tags: ["@skipSaas", "@snapPercy"] },
    () => {
      // Visit the homepage
      ...
      cy.get("select").find("option:selected").should("have.text", "Relevance");

      // Assert filter checkbox is checked
      cy.get('input[type="checkbox"][value="collections"]').should(
        "be.checked",
      );
      ...
      cy.get('input[type="checkbox"][value="collections"]').uncheck({
        force: true,
      });
      cy.contains('35 results found for "tee".');

      // Check Filter checkbox
      cy.get('input[type="checkbox"][value="apparel/shirts"]').check({
        force: true,
      });
