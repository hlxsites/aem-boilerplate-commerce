...
describe("Verify auth user can manage products across wishlist and cart", () => {
  it("Successfully add simple product to wishlist, move it to cart, return this to wishlist and remove it", () => {
    ...
    cy.get(
      '.product-details__buttons__add-to-wishlist [data-testid="wishlist-toggle"]',
    )
      .should("be.visible")
      .and("not.be.disabled");
    ...
    assertWishlistProductImage(Cypress.env("productImageName"))(
      ".commerce-wishlist-wrapper",
    );

    // Move item to cart with proper waiting
    cy.contains("Move To Cart")
      .should("be.visible")
      .and("not.be.disabled")
      .click();
    ...
    cy.get('[data-testid="wishlist-product-item-remove-button"]', {
      timeout: 15000,
    })
      .should("be.visible")
      .and("not.be.disabled")
  ...
  it(
    "Successfully add configurable product with all required options to wishlist, move it to cart and return this to wishlist",
    { tags: "@snapPercy" },
    () => {
      cy.visit("/customer/create");
      cy.fixture("userInfo").then(({ sign_up }) => {
      ...
      assertWishlistProductImage(
        Cypress.env("productWithOptionImageNameConfigurable"),
      )(".commerce-wishlist-wrapper");
