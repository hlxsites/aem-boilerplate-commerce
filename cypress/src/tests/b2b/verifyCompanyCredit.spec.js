  ...
  () => {
    ...
    it("JOURNEY: Company credit display and operations with permissions", () => {
      ...
      cy.then(() => {
        ...
        cy.contains(/outstanding.*balance/i, { timeout: 10000 }).should(
          "be.visible",
        );
        cy.contains(/available.*credit/i).should("be.visible");
        cy.contains(/credit.*limit/i).should("be.visible");
        ...
        cy.then(async () => {
          const creditInfo = await getCompanyCredit(
            Cypress.env("testCompany").id,
          );
          const creditId = creditInfo.id;
          Cypress.env("testCredit", { id: creditId });

          await increaseCompanyCreditBalance(
            creditId,
            5.0,
            "USD",
            "Test reimbursement",
        ...
        cy.contains("5.00", { timeout: 15000 }).should("be.visible");

        cy.logToTerminal('✅ Verify "Reimbursed" record in history grid');
        cy.contains(/reimburs/i, { timeout: 15000 }).should("be.visible");

        cy.logToTerminal("✅ TC-47 CASE_3: Reimbursement record verified");

        // ========== TC-47 CASE_4: Allocation operation ==========
        cy.logToTerminal(
          "--- STEP 3: TC-47 CASE_4 - Set credit limit (allocation) ---",
        );
        ...
        cy.contains("Company Credit", { timeout: 10000 }).should("be.visible");
        cy.contains(/outstanding.*balance/i).should("not.exist");
        cy.contains(/available.*credit/i).should("not.exist");
        cy.contains(/credit.*limit/i).should("not.exist");
    ...
    it("JOURNEY: Company credit with order lifecycle (Purchase, Revert, Refund)", () => {
      cy.logToTerminal(
        "========= 🚀 JOURNEY: Company Credit Order Integration =========",
      );
      ...
      cy.then(() => {
        ...
        cy.logToTerminal(
          "--- STEP 1: TC-47 CASE_1 - Place order with Payment on Account ---",
        );
        ...
        cy.contains("Checkout", { timeout: 10000 })
          .should("be.visible")
          .click();
        cy.wait(5000);
        ...
        cy.get("body").then(($body) => {
          if ($body.find('input[name="shipping_method"]').length > 0) {
            cy.get('input[name="shipping_method"]')
              .first()
              .check({ force: true });
            cy.wait(2000);
          }
        ...
        cy.get("body").then(($body) => {
          ...
          if (bodyText.includes("Payment on Account")) {
            cy.contains("Payment on Account", { timeout: 15000 }).click({
              force: true,
            });
            cy.wait(2000);
          } else {
            cy.logToTerminal(
              "⚠️ Payment on Account not found, using default payment method",
            );
          }
        });
        ...
        cy.get("body").then(($body) => {
          if (
            $body.find('input[type="checkbox"][name="terms"]').length > 0 ||
            $body.find(
              '.checkout-terms-and-conditions__form input[type="checkbox"]',
            ).length > 0
          ) {
        ...
        cy.url({ timeout: 30000 }).should(
          "match",
          /success|confirmation|order-details/,
        );
        ...
        cy.url().then((url) => {
          ...
          if (urlMatch) {
            ...
            cy.logToTerminal(
              `✅ Order placed successfully (from URL): #${orderNumber}`,
            );
          ...
          cy.get("body").then(($body) => {
            const bodyText = $body.text();
            cy.logToTerminal(
              `📄 Confirmation page content: ${bodyText.substring(0, 200)}`,
            );
            ...
            if (orderNumber) {
              Cypress.env("testOrderNumber", orderNumber);
              cy.logToTerminal(
                `✅ Order placed successfully (from content): #${orderNumber}`,
              );
            } else {
              cy.logToTerminal(
                "⚠️ Could not extract order number from confirmation page",
              );
        ...
        cy.get(".commerce-company-credit", { timeout: 15000 })
          .should("be.visible")
          .within(() => {
            cy.contains(/purchas|order/i, { timeout: 15000 }).should(
              "be.visible",
            );
          });
        cy.logToTerminal(
          "✅ TC-47 CASE_1: Purchase record verified in credit history",
        );
        ...
        cy.logToTerminal(
          "--- STEP 2: TC-47 CASE_5 - Invoice first order and create credit memo (refund) ---",
        );
        ...
        cy.then(async () => {
          ...
          if (orderNumber && orderNumber !== "unknown") {
            try {
              ...
              cy.logToTerminal(
                `✅ Invoice created for order ${orderNumber}, invoice ID: ${invoiceId}`,
              );
              cy.wait(2000);

              // Step 3: Create credit memo (triggers RefundCommand)
              cy.logToTerminal(
                `💰 Creating credit memo for order: ${orderNumber} with invoice ID: ${invoiceId}`,
              );
              const creditMemoId = await createCreditMemo(orderId, invoiceId);
              cy.logToTerminal(
                `✅ Credit memo created for order ${orderNumber}, credit memo ID: ${creditMemoId}`,
              );
              cy.logToTerminal(
                "⏳ Waiting for RefundCommand to execute and update credit history...",
              );
              cy.wait(3000);
            } catch (error) {
              cy.logToTerminal(
        ...
        const checkForRefund = () => {
          refundAttempt++;
          cy.logToTerminal(
            `🔍 Attempt ${refundAttempt}/${maxRefundRetries}: Checking for Refunded record...`,
          );
          ...
          cy.get("body").then(($body) => {
            if ($body.text().match(/refund/i)) {
              cy.logToTerminal(
                "✅ TC-47 CASE_5: Refunded record verified (via RefundCommand)",
              );
            } else if (refundAttempt < maxRefundRetries) {
              cy.logToTerminal(
                `⚠️ Refunded record not found yet, retrying... (${refundAttempt}/${maxRefundRetries})`,
              );
              cy.wait(5000); // Wait longer between retries
              checkForRefund();
            } else {
              cy.logToTerminal(
                "❌ Refunded record not found after max retries",
              );
              throw new Error(
                "Refunded record not found in credit history after credit memo creation",
              );
            }
          });
        };
        ...
        cy.logToTerminal(
          "--- STEP 3: TC-47 CASE_4 - Place second order and cancel it (revert) ---",
        );
        ...
        cy.logToTerminal(
          "💰 Selecting Payment on Account payment method for second order",
        );
        cy.get("body").then(($body) => {
          const bodyText = $body.text();
          if (bodyText.includes("Payment on Account")) {
        ...
        cy.url().then((url) => {
          ...
          if (urlMatch) {
            ...
            cy.logToTerminal(
              `✅ Second order placed successfully (from URL): #${orderNumber2}`,
            );
          ...
          cy.get("body").then(($body) => {
            const bodyText = $body.text();
            cy.logToTerminal(
              `📄 Second order confirmation page content: ${bodyText.substring(0, 200)}`,
            );
            ...
            if (orderNumber2) {
              Cypress.env("testOrderNumber2", orderNumber2);
              cy.logToTerminal(
                `✅ Second order placed successfully (from content): #${orderNumber2}`,
              );
            } else {
              cy.logToTerminal(
                "⚠️ Could not extract second order number from confirmation page",
              );
        ...
        cy.then(async () => {
          ...
          if (orderNumber2 && orderNumber2 !== "unknown") {
            try {
              ...
              const orderId = order.entity_id;
              cy.logToTerminal(
                `✅ Found order entity_id: ${orderId} for increment_id: ${orderNumber2}`,
              );

              // Cancel using entity_id
              await cancelOrder(orderId);
              cy.logToTerminal(
                `✅ Order ${orderNumber2} cancelled successfully`,
              );

              // Verify "Reverted" record in credit history
              cy.logToTerminal(
                "📊 Verifying Reverted record in credit history...",
              );
              cy.visit("/customer/company/credit");
              cy.wait(2000);
              ...
              cy.logToTerminal(
                `❌ Error cancelling order ${orderNumber2}: ${error.message}`,
              );
              throw error;
            }
          }
