import { submitQuoteToCustomer } from "../../support/b2bQuoteAPICalls";
import {
  ...
  it("Verify B2B Quote to Order Placement", { tags: "@B2BSaas" }, () => {
    ...
    cy.wrap(null)
      ...
      .then((roleResult) => {
        ...
        cy.logToTerminal(
          "========= Step 2: Create customer with quote role =========",
        );
    ...
    cy.get('input[name="firstName"]')
      .first()
      .clear()
      .type(customerData.customer.firstname);
    cy.get('input[name="lastName"]')
      .first()
      .clear()
      .type(customerData.customer.lastname);
    ...
    cy.wrap(null).then(async () => {
      try {
        ...
        if (approvalResult.success) {
          cy.logToTerminal(
            `✅ Quote approved. Quote ID: ${approvalResult.quote_id}`,
          );
    ...
    cy.logToTerminal(
      "========= Step 10: Place quote order - Click Checkout =========",
    );
    cy.wait(3000);

    cy.contains("span", "Checkout").should("be.visible").click();
    cy.wait(8000);
    ...
    cy.get(
      '[data-testid="checkout-terms-and-conditions-agreements"] input[type="checkbox"]',
    ).check({ force: true });
    cy.wait(2000);
    cy.logToTerminal("✅ Terms accepted");

    // Place Purchase Order
    cy.contains("Place Purchase Order").click({ force: true });
    cy.wait(5000);
