...
describe.skip(
  "USF-2525: Company Profile (Optimized Journeys)",
  { tags: ["@B2BSaas"] },
  () => {
    before(() => {
    ...
    it("JOURNEY: Admin views and manages company profile", () => {
      ...
      cy.then(() => {
        ...
        cy.logToTerminal(
          "--- STEP 1: TC-11 - Verify company info block (admin) ---",
        );
        ...
        cy.get(".customer-company-info-card", { timeout: 15000 }).should(
          "exist",
        );
        ...
        cy.logToTerminal(
          "--- STEP 2: TC-07/TC-08 - Verify profile display (all fields) ---",
        );
        ...
        cy.get(".account-company-profile", { timeout: 10000 }).should("exist");
        ...
        cy.logToTerminal(
          "✅ TC-07/TC-08: Profile displays correctly with all fields",
        );

        // ========== TC-12: Admin can edit profile ==========
        cy.logToTerminal(
          "--- STEP 3: TC-12 - Admin edits profile with validation ---",
        );
        ...
        cy.get(".account-edit-company-profile-form", { timeout: 5000 }).should(
          "be.visible",
        );

        // Validation: Required fields
        cy.logToTerminal("📝 Test required field validation");
        cy.get('input[name="name"]').should("be.visible").clear().blur();
        ...
        cy.contains("999 Updated Street", { timeout: 15000 }).should("exist");
        cy.contains("Updated Test Company", { timeout: 10000 }).should("exist");

        cy.logToTerminal("✅ TC-12: Admin successfully edited company profile");

        // ========== TC-14: Backend changes sync to storefront ==========
        cy.logToTerminal(
          "--- STEP 5: TC-14 - Verify backend changes sync to storefront ---",
        );

        cy.logToTerminal(
          "🔧 Updating company via REST API (simulating Admin Panel changes)...",
        );
        cy.then(async () => {
        ...
        cy.contains("BACKEND-UPDATED Company", { timeout: 15000 }).should(
          "be.visible",
        );

        cy.logToTerminal("✅ Verify backend-updated legal name");
        cy.contains("Legal BACKEND-UPDATED Company LTD").should("be.visible");

        cy.logToTerminal("✅ Verify backend-updated VAT/Tax ID");
        cy.contains("VAT-999-BACKEND").should("be.visible");

        cy.logToTerminal("✅ Verify backend-updated Reseller ID");
        cy.contains("RESELLER-999").should("be.visible");

        cy.logToTerminal("✅ Verify backend-updated city");
        cy.contains("Backend City").should("be.visible");

        cy.logToTerminal("✅ Verify backend-updated postal code");
        cy.contains("88888").should("be.visible");

        cy.logToTerminal("✅ Verify backend-updated phone");
        cy.contains("555-9999").should("be.visible");

        cy.logToTerminal(
          "✅ TC-14: Backend changes successfully synced to storefront",
        );
    ...
    it("JOURNEY: Regular user has view-only profile access", () => {
      ...
      cy.then(() => {
        ...
        cy.logToTerminal(
          "--- STEP 1: TC-11 - Verify company info block (user) ---",
        );

        // After login, user is on /customer/account
        cy.logToTerminal("✅ Verify company information block exists");
        cy.get(".customer-company-info-card", { timeout: 10000 }).should(
          "exist",
        );
        ...
        cy.logToTerminal(
          "✅ TC-11: Company info block displays for regular user",
        );
