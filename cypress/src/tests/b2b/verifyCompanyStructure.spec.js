...
describe.skip(
  "USF-2522: Company Structure (Optimized Journeys)",
  { tags: "@B2BSaas" },
  () => {
    before(() => {
    ...
    it("JOURNEY: Admin manages structure - users, teams, and hierarchy", () => {
      cy.logToTerminal(
        "========= 🚀 JOURNEY 1: Admin Full Structure Management =========",
      );
      ...
      cy.logToTerminal(
        "--- STEP 1: TC-32 - Verify default state and controls ---",
      );
      ...
      cy.contains(`${adminFirstName} ${adminLastName}`, {
        timeout: 10000,
      }).should("be.visible");
      cy.contains(`${regularUserFirstName} ${regularUserLastName}`, {
        timeout: 10000,
      }).should("be.visible");
      ...
      cy.get('input[name="team_description"]')
        .type("Sales department team")
        .blur();
      ...
      cy.logToTerminal(
        "--- STEP 3: TC-33 - Add user with unregistered email ---",
      );
      ...
      cy.get(
        '.company-user-form__card, .acm-structure-panel, [class*="user-form"]',
      )
        .first()
        .within(() => {
          cy.get('select[name="role"]', { timeout: 10000 }).select(
            "Default User",
          );
      ...
      cy.contains(/successfully.*created/i, { timeout: 10000 }).should(
        "be.visible",
      );
      ...
      cy.logToTerminal(
        "--- STEP 4: TC-32 (Part 2) - Drag & drop user into team ---",
      );
      ...
      cy.get("@dragUser").trigger("dragstart", {
      ...
      // Verify success message
      cy.contains(/successfully.*moved|moved.*successfully|user.*moved/i, {
        timeout: 5000,
      }).should("be.visible");
      ...
      // Verify success message
      cy.contains(/successfully.*updated/i, { timeout: 5000 }).should(
        "be.visible",
      );
      ...
      // Verify success message
      cy.contains(/team.*updated|successfully/i, { timeout: 5000 }).should(
        "be.visible",
      );
      ...
      cy.logToTerminal(
        "--- STEP 8: TC-39 (Part 3) - Test team hierarchy via drag & drop ---",
      );
      ...
      cy.logToTerminal(
        `🔄 Dragging "${marketingTeam}" into "${updatedTeamName}"...`,
      );

      cy.contains(marketingTeam)
        .closest(".acm-tree__item")
        .should("have.attr", "draggable", "true")
        .as("dragMarketing");
      cy.contains(updatedTeamName)
        .closest(".acm-tree__item")
        .should("have.attr", "draggable", "true")
        .as("dropSales");

      cy.get("@dragMarketing").trigger("dragstart", {
        dataTransfer: new DataTransfer(),
      });
      ...
      // Verify success message
      cy.contains(/team.*successfully.*moved|successfully.*moved/i, {
        timeout: 5000,
      }).should("be.visible");
      ...
      cy.logToTerminal(
        "--- STEP 9: TC-32 (Part 3) - Test Collapse/Expand All ---",
      );
    ...
    it("JOURNEY: Invitation flow and entity removal", () => {
      ...
      cy.logToTerminal(
        "--- STEP 1: TC-34 - Add user with registered email ---",
      );
      ...
      cy.then(async () => {
        ...
        cy.logToTerminal(
          `✅ Pre-registered customer created: ${registeredUserEmail} (ID: ${registeredUserId})`,
        );
      ...
      // Verify success message
      cy.contains(/successfully|sent|invitation/i, { timeout: 10000 }).should(
        "be.visible",
      );
      ...
      cy.contains(
        `${registeredUserFirstName} ${registeredUserLastName}`,
      ).should("not.exist");
      cy.logToTerminal(
        "✅ Verified: User not in structure (invitation pending)",
      );

      // WORKAROUND: Accept invitation via REST API
      cy.then(async () => {
        cy.logToTerminal(
          "🔗 Simulating invitation acceptance via REST API (WORKAROUND)...",
        );
      ...
      cy.contains(`${registeredUserFirstName} ${registeredUserLastName}`, {
        timeout: 10000,
      }).should("be.visible");
      ...
      cy.then(() => {
        ...
        cy.get(".dropin-modal button").then(($buttons) => {
          const removeBtn = $buttons.filter((i, el) =>
            Cypress.$(el).text().includes("Remove"),
          );
        ...
        // Verify success message
        cy.contains(/removed|inactive/i, { timeout: 5000 }).should(
          "be.visible",
        );

        // Verify user removed from tree
        cy.contains(`${regularUserFirstName} ${regularUserLastName}`).should(
          "not.exist",
        );
        ...
        cy.logToTerminal(
          "✅ TC-38: User status is Inactive in Company Users grid",
        );
      });
      ...
      cy.get(".dropin-modal button").then(($buttons) => {
        const deleteBtn = $buttons.filter((i, el) =>
          Cypress.$(el).text().includes("Delete"),
        );
      ...
      // Verify success message
      cy.contains(/team.*deleted|successfully/i, { timeout: 5000 }).should(
        "be.visible",
      );
