...
// describe.skip(
//   "Company Switcher (Optimized Journey)",
//   { tags: ["@B2BSaas"] },
//   () => {
//     before(() => {
...
//         cy.logToTerminal(
//           "🏢 Setting up two companies with shared user (admin in A, regular in B)...",
//         );
...
//           cy.logToTerminal(
//             `✅ Company A created: ${companyA.name} (ID: ${companyA.id})`,
//           );
...
//           cy.logToTerminal(
//             `✅ Company B created: ${companyB.name} (ID: ${companyB.id})`,
//           );
...
//           cy.logToTerminal(
//             `✅ Shared user created: ${sharedUser.email} (ID: ${sharedUser.id})`,
//           );
...
//           cy.logToTerminal(
//             "✅ Shared user assigned to Company B as DEFAULT USER",
//           );
...
//         cy.logToTerminal(
//           "--- STEP 1: TC-41 - Verify admin role in Company A ---",
//         );
...
//             .contains(companyAName, { timeout: 10000 })
//             .should("be.visible");
//         });
...
//           cy.get(".dropin-picker__select", { timeout: 10000 })
//             .first()
//             .select(companyBName);
//           cy.wait(3000);
...
//         cy.logToTerminal(
//           "--- STEP 3: TC-40 - Verify My Company page shows Company B data ---",
//         );
...
//           const companyBName = Cypress.env("companyBName");
//           cy.get(".account-company-profile")
//             .contains(companyBName, { timeout: 10000 })
//             .should("be.visible");
...
//         cy.logToTerminal(
//           "--- STEP 4: TC-41 - Verify regular user role in Company B ---",
//         );
...
//         cy.logToTerminal(
//           "--- STEP 5: TC-40 - Verify Company Users grid shows Company B users ---",
//         );
...
//               if (
//                 tableText.includes(adminBEmail) ||
//                 tableText.includes("CompanyB")
//               ) {
//                 cy.logToTerminal("✅ Company Users grid shows Company B users");
//                 cy.contains(/CompanyB|Shared User/, { timeout: 5000 }).should(
//                   "be.visible",
//                 );
//               } else if (retriesLeft > 0) {
//                 cy.logToTerminal(
//                   `⏳ Company B users not yet visible, retrying (${12 - retriesLeft + 1}/12)...`,
//                 );
//                 cy.wait(12000); // Increased wait time to 12 seconds
//                 cy.reload();
//                 cy.wait(3000);
//                 cy.get(".companyUsersTable", { timeout: 15000 }).should(
//                   "be.visible",
//                 );
//                 cy.get('[aria-busy="true"]', { timeout: 10000 }).should(
//                   "not.exist",
//                 );
//                 checkForCompanyBAdmin(retriesLeft - 1);
//               } else {
//                 cy.logToTerminal(
//                   `❌ Table content: ${tableText.substring(0, 200)}`,
//                 );
//                 throw new Error(
//                   "Company B admin not found in users grid after 12 retries (USF-3516 cache issue)",
//                 );
//               }
//             });
//           };
...
//         cy.logToTerminal(
//           "--- STEP 6: TC-40 - Verify Company Structure shows Company B tree ---",
//         );

//         cy.visit("/customer/company/structure");
//         cy.wait(3000);

//         cy.contains("Company Structure", { timeout: 10000 }).should(
//           "be.visible",
//         );
...
//             if (
//               bodyText.includes("CompanyB") ||
//               bodyText.includes("Shared User")
//             ) {
...
//             } else if (retriesLeft > 0) {
//               cy.logToTerminal(
//                 `⏳ Company B structure not yet visible, retrying (${12 - retriesLeft + 1}/12)...`,
//               );
//               cy.wait(12000); // Increased wait time to 12 seconds
//               cy.reload();
//               cy.wait(3000);
...
//               );
//               throw new Error(
//                 "Company B structure not found after 12 retries (USF-3516 cache issue)",
//               );
//             }
//           });
//         };
...
//         cy.logToTerminal(
//           "--- STEP 7: TC-41 - Verify Roles page shows Company B roles ---",
//         );
...
//         cy.get("body").then(($body) => {
//           if (
//             $body.find('[data-testid="role-and-permission-table"]').length > 0
//           ) {
//             cy.logToTerminal(
//               "✅ Roles page accessible (may vary by permission)",
//             );
//           } else {
//             cy.logToTerminal(
//               "✅ Roles page access restricted (expected for regular user)",
//             );
//           }
//         });
...
//         cy.logToTerminal(
//           "--- STEP 8: TC-40 - Switch back to Company A and verify context reversion ---",
//         );
...
//           cy.contains("button", "Edit", { timeout: 5000 }).should("be.visible");
//           cy.logToTerminal(
//             "✅ Context reverted to Company A with admin controls",
//           );
//         });

//         // ========== TC-42: Shopping Cart context switching ==========
//         cy.logToTerminal(
//           "--- STEP 9: TC-42 - Verify Shopping Cart is company-specific ---",
//         );
...
//             $body.text().includes("ADB169") ||
//             $body.text().includes("Pride at Adobe")
//           ) {
//             cy.logToTerminal("✅ Company A: Cart has product (ADB169)");
//           } else {
//             cy.logToTerminal(
//               "⚠️ Company A: Cart might be empty or product display differs",
//             );
//           }
//         });
...
//             cy.logToTerminal(
//               "✅ TC-42: Company B cart is empty (cart is company-specific)",
//             );
//           } else {
//             cy.logToTerminal(
//               "⚠️ TC-42: Company B cart might have items (unexpected)",
//             );
//           }
//         });
...
//         cy.get("body").then(($body) => {
//           if (
//             $body.text().includes("ADB150") ||
//             $body.text().includes("Youth Tee")
//           ) {
//             cy.logToTerminal("✅ Company B: Cart has product (ADB150)");
//           } else {
//             cy.logToTerminal(
//               "⚠️ Company B: Cart might be empty or product display differs",
//             );
//           }
//         });
...
//         cy.logToTerminal(
//           "🔄 Switching back to Company A to verify cart persistence...",
//         );
//         cy.visit("/customer/company");
//         cy.wait(2000);
...
//             $body.text().includes("ADB169") ||
//             $body.text().includes("Pride at Adobe");
//           const hasCompanyBProduct =
//             $body.text().includes("ADB150") ||
//             $body.text().includes("Youth Tee");

//           if (hasOriginalProduct && !hasCompanyBProduct) {
//             cy.logToTerminal(
//               "✅ TC-42: Company A cart preserved original product, cart context is company-specific",
//             );
//           } else if (hasOriginalProduct) {
//             cy.logToTerminal(
//               "⚠️ TC-42: Company A cart has original product but context might be mixed",
//             );
//           } else {
...
//         cy.logToTerminal(
//           "========= 🎉 JOURNEY COMPLETED: Company Switcher (TC-40, TC-41, TC-42) =========",
//         );
//       },
//     );
...
//         cy.logToTerminal(
//           "🏢 Setting up 4 companies with different statuses (APPROVED, PENDING, REJECTED, BLOCKED)...",
//         );
...
//           });
//           cy.logToTerminal(
//             `✅ Company APPROVED created: ${companyApproved.name} (ID: ${companyApproved.id})`,
//           );
...
//           });
//           cy.logToTerminal(
//             `✅ Company PENDING created: ${companyPending.name} (ID: ${companyPending.id})`,
//           );
...
//           });
//           cy.logToTerminal(
//             `✅ Company REJECTED created: ${companyRejected.name} (ID: ${companyRejected.id})`,
//           );
...
//           });
//           cy.logToTerminal(
//             `✅ Company BLOCKED created: ${companyBlocked.name} (ID: ${companyBlocked.id})`,
//           );
...
//           Cypress.env(
//             "currentTestAdminEmail",
//             companyApproved.company_admin.email,
//           );
...
//         cy.logToTerminal(
//           "--- STEP 1: USF-3555 - Verify dropdown shows only APPROVED and BLOCKED companies ---",
//         );
...
//         cy.get(".dropin-picker__select", { timeout: 15000 })
//           .should("be.visible")
//           .then(($select) => {
...
//             expect(hasApproved, "APPROVED company should be visible").to.be
//               .true;
//             cy.logToTerminal(
//               `✅ APPROVED company "${companyApprovedName}" is visible in dropdown`,
//             );
...
//             cy.logToTerminal(
//               `✅ BLOCKED company "${companyBlockedName}" is visible in dropdown`,
//             );

//             // Verify PENDING company is NOT in dropdown
//             const hasPending = options.some((opt) => opt.includes("Pending"));
//             expect(hasPending, "PENDING company should be filtered out").to.be
//               .false;
//             cy.logToTerminal(
//               `✅ PENDING company "${companyPendingName}" is correctly filtered out`,
//             );

//             // Verify REJECTED company is NOT in dropdown
//             const hasRejected = options.some((opt) => opt.includes("Rejected"));
//             expect(hasRejected, "REJECTED company should be filtered out").to.be
//               .false;
//             cy.logToTerminal(
//               `✅ REJECTED company "${companyRejectedName}" is correctly filtered out`,
//             );

//             // Verify dropdown has exactly 2 options (APPROVED + BLOCKED)
//             expect(
//               options.length,
//               "Dropdown should have exactly 2 companies",
//             ).to.equal(2);
//             cy.logToTerminal(
//               "✅ Dropdown shows exactly 2 companies (APPROVED + BLOCKED)",
//             );
//           });

//         // ========== USF-3555: Verify can switch to BLOCKED company ==========
//         cy.logToTerminal(
//           "--- STEP 2: USF-3555 - Verify user can switch to BLOCKED company ---",
//         );

//         cy.then(() => {
//           const companyBlockedName = Cypress.env("companyBlockedName");
//           cy.logToTerminal(
//             `🔄 Switching to BLOCKED company: ${companyBlockedName}`,
//           );
//           cy.get(".dropin-picker__select", { timeout: 10000 })
//             .first()
//             .select(companyBlockedName);
//           cy.wait(3000);
...
//         cy.then(() => {
//           const companyBlockedName = Cypress.env("companyBlockedName");
//           cy.get(".account-company-profile", { timeout: 15000 }).should(
//             "exist",
//           );
//           cy.get(".account-company-profile")
//             .contains(companyBlockedName, { timeout: 10000 })
//             .should("be.visible");
...
//         cy.logToTerminal(
//           "--- STEP 3: USF-3555 - Verify user can switch back to APPROVED company ---",
//         );

//         cy.then(() => {
//           const companyApprovedName = Cypress.env("companyApprovedName");
//           cy.logToTerminal(
//             `🔄 Switching back to APPROVED company: ${companyApprovedName}`,
//           );
//           cy.get(".dropin-picker__select", { timeout: 10000 })
//             .first()
//             .select(companyApprovedName);
//           cy.wait(3000);
...
//           const companyApprovedName = Cypress.env("companyApprovedName");
//           cy.get(".account-company-profile")
//             .contains(companyApprovedName, { timeout: 10000 })
//             .should("be.visible");
//           cy.logToTerminal("✅ Company profile shows APPROVED company data");
//         });

//         cy.logToTerminal(
//           "========= 🎉 JOURNEY COMPLETED: Company Status Filtering (USF-3555) =========",
//         );
//       },
//     );
