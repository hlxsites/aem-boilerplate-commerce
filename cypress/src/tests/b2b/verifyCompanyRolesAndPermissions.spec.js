...
describe(
  ...
  () => {
    before(() => {
      cy.logToTerminal(
        "🎭 Roles and Permissions test suite started (OPTIMIZED)",
      );
    });
    ...
    it("JOURNEY: Role management - complete lifecycle", () => {
      ...
      cy.then(() => {
        ...
        cy.contains("Company Roles & Permissions", { timeout: 10000 }).should(
          "be.visible",
        );
        ...
        cy.contains("button", "Add New Role", { timeout: 5000 }).should(
          "be.visible",
        );
        ...
        cy.contains("Default User")
          .parents("tr")
          .within(() => {
            cy.contains("button", "Edit", { timeout: 5000 }).should(
              "be.visible",
            );
            cy.contains("button", "Duplicate", { timeout: 5000 }).should(
              "be.visible",
            );
        ...
        cy.contains(/role.*created|successfully/i, { timeout: 5000 }).should(
          "be.visible",
        );
        ...
        cy.get('[data-testid="role-and-permission-table"]')
          ...
          .parent("tr")
          .within(() => {
            cy.contains("button", "Delete", { timeout: 5000 }).should(
              "be.visible",
            );
          });
        ...
        cy.logToTerminal(
          "--- STEP 3: TC-29 - Role deletion with/without users ---",
        );
        ...
        cy.logToTerminal(
          "❌ Verify Delete button not visible for role with users",
        );
        cy.get('[data-testid="role-and-permission-table"]')
          .contains("td", "Default User")
          .parent("tr")
        ...
        cy.logToTerminal(
          "✅ Delete button correctly hidden for role with assigned users",
        );

        // Create a new role without users
        cy.logToTerminal("📋 Create a new role (no users assigned)");
        cy.contains("button", "Add New Role", { timeout: 5000 }).click();

        cy.wait(1000);

        cy.get('input[name="roleName"]').type("Empty Test Role");
        ...
        cy.logToTerminal(
          "✅ TC-29: Role deletion rules verified (with users blocked, without users allowed)",
        );
    ...
    it("JOURNEY: Permission changes affect user access", () => {
      ...
      cy.then(() => {
        ...
        cy.logToTerminal(
          "--- STEP 1: TC-28 - Remove permission → verify restricted access ---",
        );
        ...
        cy.logToTerminal(
          "✅ TC-28: Permission removal successfully restricted access",
        );

        // ========== TC-30: Add "Edit Company Profile" permission ==========
        cy.logToTerminal(
          "--- STEP 2: TC-30 - Add edit permission → verify can edit ---",
        );
        ...
        cy.logToTerminal(
          '✅ Enable "Edit" permission under Company Profile > Account Information',
        );
        cy.get(".edit-role-and-permission__tree-container")
        ...
        cy.get(".account-edit-company-profile-form", { timeout: 15000 }).should(
          "not.exist",
        );

        cy.logToTerminal("✅ Verify changes are saved and displayed");
        cy.contains(updatedStreet, { timeout: 15000 }).should("exist");

        cy.logToTerminal(
          "✅ TC-30: User with edit permission can successfully edit company profile",
        );

        // ========== TC-31: Add "Manage Roles" permission ==========
        cy.logToTerminal(
          "--- STEP 3: TC-31 - Add manage roles permission → verify access ---",
        );
        ...
        cy.get("body").then(($body) => {
          if (
            ...
            cy.logToTerminal(
              "⚠️ Page visible but may have restricted functionality",
            );
          }
        });
        ...
        cy.logToTerminal(
          "✏️ Edit Default User role to add manage roles permission",
        );
        cy.get('[data-testid="role-and-permission-table"]')
          .contains("td", "Default User")
          .parent("tr")
        ...
        // Login as regular user
        cy.logToTerminal(
          "🔐 Login as regular user (now has manage roles permission)",
        );
        cy.loginAsRegularUser();
