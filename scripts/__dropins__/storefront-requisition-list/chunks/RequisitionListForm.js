...
const RequisitionListForm$1 = ({
  ...
  const title = mode === "update" ? translations.updateTitle : translations.createTitle;
  return u("div", {
    ...props,
...
const createRequisitionList = async (name, description) => {
  return fetchGraphQl(CREATE_REQUISITION_LIST_MUTATION, {
    ...
    const payload = transformRequisitionList(data.createRequisitionList.requisition_list);
...
const updateRequisitionList = async (requisitionListUid, name, description) => {
  return fetchGraphQl(UPDATE_REQUISITION_LIST_MUTATION, {
    ...
    var _a;
    if (errors) return handleFetchError(errors);
    const payload = transformRequisitionList((_a = data == null ? void 0 : data.updateRequisitionList) == null ? void 0 : _a.requisition_list);
    events.emit("requisitionList/data", payload);
    return payload;
  });
};
function useRequisitionListForm(mode, requisitionListUid, onSuccess, onError) {
  ...
  const submit = async (values) => {
    ...
    try {
      ...
      const result = mode === "update" && requisitionListUid ? await updateRequisitionList(requisitionListUid, values.name, description) : await createRequisitionList(values.name, description);
